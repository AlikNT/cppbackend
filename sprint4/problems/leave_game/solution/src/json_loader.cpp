...
double ParseDogRetirementTime(json::object &root) {
    constexpr double DEFAULT_DOG_RETIREMENT_TIME = 1.0;
    double dog_retirement_time = DEFAULT_DOG_RETIREMENT_TIME;  // Значение по умолчанию
    if (root.contains("dogRetirementTime"s)) {
         dog_retirement_time = root["dogRetirementTime"s].as_double();
    }
...
model::Game LoadGame(const std::filesystem::path& json_path) {
    ...
    double dog_retirement_time = ParseDogRetirementTime(root);
    game.AddDogRetirementTime(dog_retirement_time);
